...
import pyomo.core.base.expression as expr
...
for exp in mod_object.model.component_objects(pyo.Expression, descend_into=True):
    if "Cost" in exp.name or "cost" in exp.name:
        if type(exp) is expr.ScalarExpression:
            costs[exp.name] = pyo.value(exp)
        if type(exp) is expr.IndexedExpression:
            for e in exp:
...
folder_name = "congestion_informs"
...
load_shed_name = f"ios_results/load_shed/load_shed_{start_date}.json" #folder_name + 
...
with open(f"ios_results/power_flows/power_flows_{start_date}.json", "w") as fil:
    json.dump(flows, fil)
...
with open(f"ios_results/max_flows/max_flows_{start_date}.json", "w") as fil:
